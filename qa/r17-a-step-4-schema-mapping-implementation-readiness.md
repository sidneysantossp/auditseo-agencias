...
| **Evidence** | `EvidenceItem` contém apenas `type`, `statement`, `source?`, `limitations?` e `notes?`; `EvidenceCategory` aceita `DOCUMENTADO`, `OBSERVADO`, `TESTADO`, `INFERIDO` e `RECOMENDADO` [1]. | `COMPATIBILITY DEBT` | Os itens atuais podem continuar sendo exibidos e preservados como registro editorial legado. `DOCUMENTADO`, `OBSERVADO` e `TESTADO` são compatíveis com source-bearing evidence quando o contexto disponível for suficiente. | Separar Evidence Item source-bearing de Interpretation e Recommendation; as cinco categorias atuais permanecem Compatibility Debt e não são destino conceitual. |
...
| **Missingness** | `source?`, `limitations?`, `notes?`, arrays/strings e status/contexto opcionais; não existe enum explícito de missingness no contrato atual [1] [4]. | `EXTENSION REQUIRED` | Ausências e gaps podem ser preservados como ausência de evidência, sem promoção epistemológica. | O modelo-alvo requer `MISSING`, `UNCLEAR` e `NOT_ASSESSABLE`; nunca converter ausência em zero, `NO`, score ou inferência negativa. |
| **Raw Capture Reference** | Não existe objeto estável de captura bruta; `source?`, `references?`, query, system, date e `resultSummary` estão disponíveis apenas parcialmente [1] [4]. | `EXTENSION REQUIRED` | URLs, referências e contexto observável podem permanecer como matéria-prima de provenance. | Captura formal futura: referência reproduzível, surface, locale, timestamp e payload/texto quando disponíveis; ausência permanece `MISSING`/`NOT_ASSESSABLE`. |
