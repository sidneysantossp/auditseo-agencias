...
| **Evidence** | `EvidenceItem`: `type`, `statement`, `source?`, `limitations?`, `notes?`; cinco categorias `EvidenceCategory` [1]. | `COMPATIBILITY DEBT` | Preservar a exibição legada; `DOCUMENTADO`, `OBSERVADO` e `TESTADO` podem sustentar source-bearing evidence quando houver contexto suficiente. | Separar Evidence source-bearing de Interpretation/Recommendation; as cinco categorias legadas permanecem dívida de compatibilidade. |
...
| **Raw Capture Reference** | Não há objeto estável de captura; `source?`, `references?`, query, system, date e `resultSummary` são parciais [1] [4]. | `EXTENSION REQUIRED` | URLs, referências e contexto observável permanecem matéria-prima de provenance. | Captura futura exige referência reproduzível, surface, locale, timestamp e payload/texto quando disponíveis; ausência permanece `MISSING`/`NOT_ASSESSABLE`. |
