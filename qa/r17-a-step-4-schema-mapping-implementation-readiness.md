...
| **Raw Capture Reference** | Não existe objeto estável de captura bruta; `source?`, `references?`, query, system, date e `resultSummary` estão disponíveis apenas parcialmente [1] [4]. | `EXTENSION REQUIRED` | URLs, referências e contexto observável podem permanecer como matéria-prima de provenance. | Uma captura formal exigiria referência reproduzível, surface, locale, timestamp, query/sistema e payload ou texto observado quando disponível; ausência permanece `MISSING`/`NOT_ASSESSABLE`, sem captura inventada. |
